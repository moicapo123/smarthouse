...
export default function Index({ records, filters, success, error }: Props) {    
    ...
    const handleDragEnd = (event: any) => {
        ...
        if (active.id !== over.id) {
            setBanners((items) => {
                ...
                // Enviar petición PUT
                router.put(route('banners.reorder'), { products: newItems.map(item => item.id) }, {
                    onSuccess: () => {
    ...
    const handleTogglePublish = (banner: Banner) => {
        router.patch(route('banners.toggle-publish', banner.id), {}, {
            onSuccess: () => {
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-6">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-2xl font-bold mb-3">Banners</h1>
                        <p className="text-muted-foreground">Gestiona los banners del  sistema</p>                        
                    </div>                    
                ...
                <Card>
                    <CardHeader>
                        <div className="flex items-center justify-between">
                            <div>
                                <CardTitle className='mb-3'>Lista de banners</CardTitle>
                                <CardDescription>
                                    <small>Gestiona los baners del sistema ({records.total} baners)</small>                                    
                                </CardDescription>
                            </div>
                        </div>
                    </CardHeader>
